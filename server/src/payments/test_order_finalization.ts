...
async function setupDb() {
  ...
  await db.execute("DELETE FROM outbox_events WHERE aggregate_type = 'ORDER'");
  await db.execute('DELETE FROM print_jobs WHERE student_id = ?', [STUDENT_ID]);
  await db.execute('DELETE FROM invoices WHERE student_id = ?', [STUDENT_ID]);
  ...
  await db.execute('DELETE FROM users WHERE id IN (?, ?)', [STUDENT_ID, MANAGER_ID]);
